#!/usr/bin/env node
/**
 * Search ICICIBANK Documents for Verifiable Promises
 */
...
const testIcicibankPromises = async () => {
  console.log('\n' + '='.repeat(80));
  console.log('ICICIBANK PROMISE SEARCH');
  ...
  const browserHeaders = {
    ...
    'Referer': 'https://www.icicibank.com/about-us/investor-relations',
    'Connection': 'keep-alive'
  };

  // Get ICICIBANK IR page
  console.log('STEP 1: Fetching ICICIBANK IR page...');
  let pdfUrls = [];
  
  try {
    const irResponse = await axios.get('https://www.icicibank.com/about-us/investor-relations', {
      timeout: 15000,
      maxRedirects: 5,
    ...
    for (const match of matches) {
      ...
      if (lowerUrl.includes('fy24') || lowerUrl.includes('fy25') || lowerUrl.includes('fy2024') || lowerUrl.includes('fy2025') || lowerUrl.includes('annual') || lowerUrl.includes('quarterly') || lowerUrl.includes('result') || lowerUrl.includes('earnings')) {
        if (!url.startsWith('http')) {
          if (url.startsWith('/')) {
            url = 'https://www.icicibank.com' + url;
          } else {
            url = 'https://www.icicibank.com/about-us/investor-relations/' + url;
          }
        }
        pdfUrls.push(url);
  ...
  for (let i = 0; i < Math.min(2, pdfUrls.length); i++) {
    ...
    try {
      ...
      // Extract first 15 pages for quick scan
      for (let j = 1; j <= Math.min(15, numPages); j++) {
    ...
    const document = {
      sourceUrl: pdfUrl,
      sourceName: `ICICIBANK Document ${i + 1}`,
      sourceDate: new Date().toISOString(),
      title: `ICICIBANK Document ${i + 1}`,
      pages: pages
    };
