#!/usr/bin/env node
/**
 * Search HDFCBANK Documents for Verifiable Promises
 */
...
const testHdfcbankPromises = async () => {
  console.log('\n' + '='.repeat(80));
  console.log('HDFCBANK PROMISE SEARCH');
  ...
  // Get HDFCBANK IR page
  console.log('STEP 1: Fetching HDFCBANK IR page...');
  let pdfUrls = [];
      ...
      if (lowerUrl.includes('fy24') || lowerUrl.includes('fy25') || lowerUrl.includes('fy2024') || lowerUrl.includes('fy2025') || lowerUrl.includes('annual') || lowerUrl.includes('quarterly') || lowerUrl.includes('result') || lowerUrl.includes('earnings')) {
        if (!url.startsWith('http')) {
          if (url.startsWith('/')) {
            ...
            url = 'https://www.hdfcbank.com/personal/about-us/investor-relations/' + url;
          }
        }
        pdfUrls.push(url);
  ...
  const allPromises = [];
  for (let i = 0; i < Math.min(3, pdfUrls.length); i++) {
    ...
    const document = {
      sourceUrl: pdfUrl,
      sourceName: `HDFCBANK Document ${i + 1}`,
      sourceDate: new Date().toISOString(),
      title: `HDFCBANK Document ${i + 1}`,
      pages: pages
    };
