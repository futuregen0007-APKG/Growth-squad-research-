#!/usr/bin/env node
/**
 * Search INFY Documents for Verifiable Promises
 */
...
const testInfyPromises = async () => {
  console.log('\n' + '='.repeat(80));
  console.log('INFY PROMISE SEARCH');
  ...
  const browserHeaders = {
    ...
    'Referer': 'https://www.infosys.com/investors.html',
    'Connection': 'keep-alive'
  };

  // Get INFY IR page
  console.log('STEP 1: Fetching INFY IR page...');
  let pdfUrls = [];
  
  try {
    const irResponse = await axios.get('https://www.infosys.com/investors.html', {
      timeout: 15000,
      maxRedirects: 5,
    ...
    for (const match of matches) {
      ...
      if (lowerUrl.includes('fy24') || lowerUrl.includes('fy25') || lowerUrl.includes('fy2024') || lowerUrl.includes('fy2025') || lowerUrl.includes('annual') || lowerUrl.includes('quarterly') || lowerUrl.includes('result') || lowerUrl.includes('earnings')) {
        if (!url.startsWith('http')) {
          if (url.startsWith('/')) {
            url = 'https://www.infosys.com' + url;
          } else {
            url = 'https://www.infosys.com/investors/' + url;
          }
        }
        pdfUrls.push(url);
    ...
    // If no specific PDFs found, use all available PDFs
    if (pdfUrls.length === 0 && matches.length > 0) {
      for (const match of matches) {
        ...
        pdfUrls.push(url);
        console.log(`✓ Using PDF: ${url.substring(0, 80)}...`);
      }
    }
  ...
  for (let i = 0; i < Math.min(2, pdfUrls.length); i++) {
    ...
    try {
      ...
      // Extract first 20 pages for quick scan
      for (let j = 1; j <= Math.min(20, numPages); j++) {
    ...
    const document = {
      sourceUrl: pdfUrl,
      sourceName: `INFY Document ${i + 1}`,
      sourceDate: new Date().toISOString(),
      title: `INFY Document ${i + 1}`,
      pages: pages
    };
