...
import { Newspaper, Clock, ExternalLink, TrendingUp, Filter, Search } from "lucide-react";
  ...
  const filteredNews = useMemo(() => {
    return NEWS_FEED.filter(news => {
      const matchesSearch = searchQuery === "" || 
        ...
        news.tickers.some(t => t.toLowerCase().includes(searchQuery.toLowerCase()));
      
      const matchesCategory = categoryFilter === "all" || news.category === categoryFilter;
      const matchesStock = stockFilter === "all" || news.tickers.includes(stockFilter);
      
      return matchesSearch && matchesCategory && matchesStock;
    });
  }, [searchQuery, categoryFilter, stockFilter]);

  const categories = ["all", "market", "company", "earnings", "economy", "global"];

  const newsByCategory = useMemo(() => {
    return categories.reduce((acc, cat) => {
      acc[cat] = filteredNews.filter(n => cat === "all" || n.category === cat);
      return acc;
    }, {});
  }, [filteredNews]);
  ...
  return (
    <div className="gs-card p-4 hover:bg-gs-cardHover transition-colors cursor-pointer group">
      <div className="flex items-start justify-between mb-2">
        <Badge variant="outline" className="text-[10px] uppercase tracking-wider border-gs-border text-gs-textDim">
          {news.category}
        </Badge>
        ...
        {news.tickers.length > 0 && (
          <div className="flex gap-1">
            {news.tickers.slice(0, 2).map(ticker => (
              <Badge key={ticker} variant="secondary" className="text-[10px] font-mono bg-gs-panel border-gs-border text-gs-textMuted">
                {ticker}
              </Badge>
            ))}
            {news.tickers.length > 2 && (
              <Badge variant="secondary" className="text-[10px] bg-gs-panel border-gs-border text-gs-textDim">
                +{news.tickers.length - 2}
              </Badge>
            )}
          </div>
